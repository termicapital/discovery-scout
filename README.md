...
| 0.6.1 | Phase 3b upgraded from `sonar` to `sonar-deep-research` as default — runs 50-70 internal searches per call vs 1 search, returns 40-50 cited sources vs ~9, finds named customer-vendor pairings the cheaper model misses. Cost goes from ~$0.05/run to ~$0.65/run (still ~$2.50-3.50/month at typical cadence). 2026-06-10 A/B test on Donor CRM confirmed material quality lift (Naseej-MEDAD adoption at Darah found by Deep Research, missed by Sonar). |
| 0.6.2 | New Phase 3c — Social Signal Scan (X / Saudi Arabia). Three Perplexity Search API calls with `x.com` domain filter + `country: "SA"`: pain validation, regulator voice, competitive sentiment. Surfaces Arabic + English Saudi X content (cached / older content well-covered; X's auth-wall limits recent coverage). 2026-06-11 test on Donor CRM surfaced NCNP enforcement actions, HRSD platform migration, official penalty schedules. ~$0.015 incremental cost per venture run. X API Basic ($100/mo) flagged as v0.7.0 upgrade path for engagement metrics + real-time. |
| 0.7.0 | Phase 3c expanded into a two-layer "Social + Competitive Recency Layer." **Layer 1 (Perplexity-X, unchanged):** cheap historical/regulatory signal via `search_domain_filter`. **Layer 2 (Grok-Top via xAI x_search, NEW):** native X access with `mode: "Top"` for relevance-ranked results, surfaces posts from <48h ago that Perplexity's cache misses. 2026-06-11 A/B test on Donor CRM proved Grok-Top found Naseej-Zoho partnership (Jun 8, 2026) and Inaya Medical College MEDAD adoption (Jun 10, 2026) — competitive intel neither Perplexity nor Deep Research captured. **Critical framing update:** Phase 3c is explicitly NOT problem validation (Stage 1.2 interviews remain that). Phase 3c surfaces *competitive + regulatory recency*. Activated by XAI_API_KEY env var; cost ~$1.20/call. Future LinkedIn-based Phase 3d flagged as v0.8.x candidate for actual voice-of-customer. |
| 0.7.1 | Readable-cold rule strengthened. **Mandatory "Cast of Characters" table** near the top of every brief — lists every named entity (regulator, sovereign platform, vendor, competitor, customer-target, X handle) with `Entity / Type / Description / Why it matters`. Revisions cannot reference entities not already in the table. **First-mention parenthetical per major subsection** (was once-per-brief): every section is now standalone-readable. **Research-trace per finding** mandatory: every claim anchored to the tool that found it + confidence band + source link. Addresses 2026-06-11 user feedback that revisions (v0.6.x, v0.7.0) added entity references without explanation, forcing readers to do external research to understand the brief. |
| 0.7.3 | **Grok-Top always-on.** Phase 3c Layer 2 no longer gated behind the `--ic-bound` flag — it runs in EVERY run where `XAI_API_KEY` is set. Track-record rationale: every Grok execution to date surfaced decisive intel no other tool found (Naseej-Zoho partnership; Aramco SAR 45M + SABIC SAR 10M Ehsan donations; Energy Sector Fund's pre-public status). Cost-conscious gating proved false economy at ~$0.40–1.20/call. Per-run cost now ~$1.50–2.50 (~$6–10/month at typical cadence). |
| 0.7.2 | Consistency cleanup — resolved internal contradictions accumulated across versions. **Q1–Q16 scoring contradiction fixed in 4 places** (Purpose, Quality Controls, "What this skill does NOT do", 5.3 table all still said scoring was forbidden — v0.2.0's Phase 5b made it mandatory; an autonomous run could behave unpredictably depending on which section it read first). Frontmatter description updated (was still "draft-only"); **Notion MCP tools added to allowed-tools** (writes would fail under enforcement); Inputs section made mode-aware (autonomous never asks, picks least-recently-run unsaturated edge sector when no ministry given); **Windows-safe bash patterns** (working-directory files not /tmp, --data-binary @file, UTF-8 parsing — each rule traceable to a real failure that cost money); Completion Report extended with all v0.2.0+ fields (composite, scores, MVP test, verdict, rank, research cost); stale facts removed; .gitignore added for API test artifacts. |
